...
function build(spec: PluginSpec): ObsidianStylePlugin {
  const perms = (spec.perms ?? ["commands", "readBlocks", "writeBlocks"]) as Perm[];
  return {
    manifest: {
      id: "com.logseqrs." + spec.id,
      name: spec.name,
      version: "0.1.0",
      description: spec.tagline + " · 原生实现，MIT 协议开源（仿 Obsidian 社区插件理念，全部使用本应用提供的原生 API）。",
      author: "全视维 官方",
      entry: "main.js",
      permissions: perms,
      kind: "native",
...
const SPECS: PluginSpec[] = [
  {
    ...
    body: String.raw`
logseq.commands.register("count-today", "字数统计：今日 journal", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("reading-time-today", "阅读时长：今日 journal", safe(async function () {
  const blocks = await journalBlocks();
  let chars = 0;
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("toc-today", "目录：生成今日 journal 目录", safe(async function () {
  const blocks = await journalBlocks();
  const lines = [];
  for (const b of blocks) {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("sort-today-lines", "行排序：今日 journal 所有块（升序）", safe(async function () {
  const blocks = await journalBlocks();
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("unique-today-lines", "行去重：今日 journal", safe(async function () {
  const blocks = await journalBlocks();
  const seen = new Set();
  const out = [];
  for (const b of blocks) {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("case-demo", "大小写：写入对照示例", safe(async function () {
...
logseq.commands.register("emoji-sheet", "Emoji：插入速查表", safe(async function () {
...
logseq.commands.register("lorem-en", "Lorem：英文占位", safe(async function () { await appendToday(EN); }));
logseq.commands.register("lorem-zh", "Lorem：中文占位", safe(async function () { await appendToday(ZH); }));`,
  },
  {
    id: "ascii-banner",
    name: "ASCII 横幅",
...
logseq.commands.register("banner-day", "ASCII：今日横幅", safe(async function () { await appendToday(banner(dateStr())); }));
logseq.commands.register("banner-focus", "ASCII：FOCUS 横幅", safe(async function () { await appendToday(banner("FOCUS")); }));`,
  },
  {
    id: "checklist-toggle",
    name: "Checklist 统计",
    ...
    body: String.raw`
logseq.commands.register("checklist-stats", "Checklist：今日完成率", safe(async function () {
  const blocks = await journalBlocks();
  let total = 0, done = 0;
  for (const b of blocks) {
...
logseq.commands.register("pomo-25", "番茄钟：开始 25 分钟专注", function () { start("专注", 25); });
logseq.commands.register("pomo-5", "番茄钟：开始 5 分钟休息", function () { start("休息", 5); });
logseq.commands.register("pomo-stop", "番茄钟：停止当前计时", function () { if (timer) { clearTimeout(timer); timer = null; notify("已停止番茄钟"); } else notify("当前没有计时"); });`,
  },
  {
    id: "habit-tracker",
    name: "习惯打卡",
    ...
    body: String.raw`
logseq.commands.register("habit-checkin", "习惯：今日三项打卡", safe(async function () {
  await appendToday("**今日打卡**\n- [ ] 📖 阅读\n- [ ] 🏃 运动\n- [ ] 🧘 冥想");
  notify("已写入今日打卡");
}));
logseq.commands.register("habit-streak", "习惯：扫描连续打卡天数", safe(async function () {
  const pages = (await logseq.api.listPages()) || [];
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("frog-today", "GTD：写入「今日青蛙」", safe(async function () {
...
logseq.commands.register("eisenhower", "GTD：插入艾森豪威尔矩阵", safe(async function () { await appendToday(M); }));`,
  },
  {
    id: "weekly-review",
    name: "每周复盘",
...
logseq.commands.register("weekly-review", "GTD：插入每周复盘", safe(async function () {
...
logseq.commands.register("okr-insert", "OKR：插入模板", safe(async function () {
...
logseq.commands.register("swot-insert", "SWOT：插入模板", safe(async function () { await appendToday(SWOT); }));`,
  },
  {
    id: "one-on-one",
    name: "1:1 会议",
...
logseq.commands.register("one-on-one", "1:1：插入模板", safe(async function () {
  ...
  // ----------------- 日记 -----------------
  {
    ...
    body: String.raw`
logseq.commands.register("on-this-day", "日记：On This Day", safe(async function () {
  const now = new Date();
  const lines = [];
  for (let i = 1; i <= 5; i++) {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("five-year-journal", "日记：五年同日", safe(async function () {
  const now = new Date();
  const lines = [];
  for (let i = 0; i < 5; i++) {
    const d = new Date(now); d.setFullYear(d.getFullYear() - i);
...
for (let i = 1; i <= 5; i++) {
  (function (n) {
    logseq.commands.register("mood-" + n, "心情：今日 " + n + " 分", safe(async function () { await appendToday(mk(n)); notify("已记录心情：" + n + "/5"); }));
  })(i);
}`,
  },
  {
...
logseq.commands.register("gratitude-3", "感恩：插入三件事", safe(async function () { await appendToday(G); }));`,
  },
  {
    id: "random-prompt",
    name: "写作灵感",
...
logseq.commands.register("prompt-random", "灵感：随机写作题目", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("weather-stamp", "天气：写入今日天气", safe(async function () {
  ...
  },
  {
    ...
    perms: ["commands", "readBlocks", "writeBlocks", "http"],
    body: String.raw`
logseq.commands.register("quote-of-day", "金句：写入今日一句", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("this-year-pages", "导航：本年度 journal 概览", safe(async function () {
  ...
  // ----------------- 引用 / 反链 -----------------
  {
    ...
    body: String.raw`
logseq.commands.register("random-note", "导航：打开随机笔记", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("orphan-pages", "导航：扫描孤立页面", safe(async function () {
  const pages = ((await logseq.api.listPages()) || []).filter(function (p) { return !p.journal_day; });
  const orphans = [];
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("dead-links-today", "扫描：今日 journal 无效双链", safe(async function () {
  const pages = (await logseq.api.listPages()) || [];
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("tag-cloud", "导航：生成标签云", safe(async function () {
  const blocks = await journalBlocks();
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("backlink-count", "反链：当前页计数", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("recent-edits", "导航：最近编辑 30 个页面", safe(async function () {
  const pages = ((await logseq.api.listPages()) || [])
  ...
  // ----------------- 查询 / 视图 -----------------
  {
    ...
    body: String.raw`
logseq.commands.register("tasks-by-tag-important", "任务：列出 #important", safe(async function () {
...
logseq.commands.register("tasks-by-tag-project", "任务：列出 #project", safe(async function () {
  const all = (await logseq.api.openTasks()) || [];
...
logseq.commands.register("kanban-insert", "看板：插入三栏模板", safe(async function () { await appendToday(K); }));`,
  },
  {
    id: "today-mentions",
    name: "今日提及",
    ...
    body: String.raw`
logseq.commands.register("today-mentions", "查询：被提及（YYYY-MM-DD）", safe(async function () {
  const q = dateStr();
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("status-summary", "查询：任务状态分布", safe(async function () {
  const all = (await logseq.api.openTasks()) || [];
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("quick-stats", "统计：知识库一览", safe(async function () {
  const pages = (await logseq.api.listPages()) || [];
...
logseq.commands.register("search-todo", "搜索：TODO", safe(function () { return run("TODO"); }));
logseq.commands.register("search-doing", "搜索：DOING", safe(function () { return run("DOING"); }));
logseq.commands.register("search-question", "搜索：?? 待澄清", safe(function () { return run("??"); }));`,
  },
  ...
  // ----------------- 白板 -----------------
  {
    ...
    body: String.raw`
logseq.commands.register("mindmap-new", "白板：新建思维导图", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("flowchart-new", "白板：新建流程图", safe(async function () {
  ...
  },
  {
    ...
    tagline: "新建白板用作今日看板",
    body: String.raw`
logseq.commands.register("kanban-board-new", "白板：新建今日看板", safe(async function () {
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("orgchart-new", "白板：新建组织图", safe(async function () {
...
logseq.commands.register("clip-example", "网络：剪藏 example.com", safe(function () { return clip("https://example.com"); }));
logseq.commands.register("clip-tauri", "网络：剪藏 tauri.app", safe(function () { return clip("https://tauri.app"); }));`,
  },
  {
    id: "youtube-notes",
    name: "YouTube 笔记",
...
logseq.commands.register("yt-notes", "模板：YouTube 笔记", safe(async function () { await appendToday(Y.replace("{DATE}", dateStr())); }));`,
  },
  {
    id: "github-repo-info",
    name: "GitHub 仓库信息",
...
logseq.commands.register("gh-self", "GitHub：查看本仓库", safe(function () { return info("jabc9543-byte/-"); }));`,
  },
  {
...
logseq.commands.register("wiki-km", "维基：知识管理", safe(function () { return wiki("知识管理"); }));
logseq.commands.register("wiki-pkm", "维基：个人知识管理", safe(function () { return wiki("个人知识管理"); }));`,
  },
  {
    id: "hacker-news-top",
    ...
    perms: ["commands", "readBlocks", "writeBlocks", "http"],
    body: String.raw`
logseq.commands.register("hn-top", "Hacker News：Top 10", safe(async function () {
...
logseq.commands.register("meeting-notes", "模板：会议纪要", safe(async function () { await appendToday(T.replace("{DATE}", dateStr())); }));`,
  },
  {
    id: "book-notes",
    name: "读书笔记",
...
logseq.commands.register("book-notes", "模板：读书笔记", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "movie-notes",
    name: "观影记录",
...
logseq.commands.register("movie-notes", "模板：观影记录", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "recipe-notes",
    name: "食谱卡片",
...
logseq.commands.register("recipe-notes", "模板：食谱", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "project-brief",
    name: "项目简报",
...
logseq.commands.register("project-brief", "模板：项目简报", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "bug-report",
    name: "Bug 报告",
...
logseq.commands.register("bug-report", "模板：Bug 报告", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "cornell-notes",
    name: "康奈尔笔记",
...
logseq.commands.register("cornell-notes", "模板：康奈尔笔记", safe(async function () { await appendToday(T); }));`,
  },
  {
    id: "zettelkasten",
    name: "Zettel 卡片",
...
logseq.commands.register("zettel-new", "模板：Zettel 卡片", safe(async function () { await appendToday(T.replace("{DATE}", dateStr())); }));`,
  },
  ...
  // ----------------- UI 快捷 -----------------
  {
    ...
    body: String.raw`
logseq.commands.register("jump-yesterday", "跳转：昨天", safe(async function () { await jumpJournal(shiftDate(new Date(), -1)); }));`,
  },
  {
    id: "jump-tomorrow",
    name: "跳转明天",
    ...
    body: String.raw`
logseq.commands.register("jump-tomorrow", "跳转：明天", safe(async function () { await jumpJournal(shiftDate(new Date(), 1)); }));`,
  },
  {
    id: "jump-last-week",
    name: "跳转上周今天",
    ...
    body: String.raw`
logseq.commands.register("jump-last-week", "跳转：7 天前", safe(async function () { await jumpJournal(shiftDate(new Date(), -7)); }));`,
  },
  {
    id: "random-journal",
    name: "随机回顾",
    ...
    body: String.raw`
logseq.commands.register("random-journal", "跳转：随机回顾", safe(async function () {
  const pages = ((await logseq.api.listPages()) || []).filter(function (p) { return p.journal_day; });
  ...
  },
  {
    ...
    body: String.raw`
logseq.commands.register("export-today", "导出：今日 journal 拼接为 Markdown 块", safe(async function () {
  const blocks = await journalBlocks();
